...
impl KernelStack {
    ...
    pub fn push_context(&self, cx: TrapContext) -> &'static mut TrapContext {
        let cx_ptr = (self.top() - core::mem::size_of::<TrapContext>()) as *mut TrapContext;
        unsafe {
            *cx_ptr = cx;
        }
...
lazy_static! {
    pub(crate) static ref APP_MANAGER: UPSafeCell<AppManager> = unsafe {
        UPSafeCell::new({
            extern "C" {
                fn _num_app();
            }
...
impl AppManager {
    pub(crate) fn print_app_info(&self) {
        println!("[kernel] Number of applications: {}", self.num_app);
        for i in 0..self.num_app {
            println!(
                "[kernel] app_{}: 0x{:08x} - 0x{:08x}",
